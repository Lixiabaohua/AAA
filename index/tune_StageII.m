function [const,Pbeta,Pnbeta,mybic,coefind1,c1,c2,coeff2 ]= tune_StageII(Palp,k,m,mu,sampleX,y,bound,p,delta)
...
const = Palp(:,1) + Palp(:,2)*c(1)+Palp(:,3)*c(2);
...
varyterm = length(Ind);
fit = const+ Palp(:,2).*Pbeta(:,1)+Palp(:,3).*Pbeta(:,2) ;
rss2 = sum((y- fit ).^2);    
